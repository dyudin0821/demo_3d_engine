...
class Object3D:
    ...
    def screen_projection(self):
        ...
        vertexes = vertexes @ self.render.projections.projection_matrix
        vertexes /= vertexes[:, -1].reshape(-1, 1)
        vertexes[(vertexes > 2) | (vertexes < -2)] = 0
        vertexes = vertexes @ self.render.projections.to_screen_matrix
        vertexes = vertexes[:, :2]
        ...
        for vertex in vertexes:
            if not np.any((vertex == self.render.H_WIDTH) | (vertex == self.render.H_HEIGHT)):
                pg.draw.circle(self.render.screen, pg.Color('Orange'), vertex, 3)
